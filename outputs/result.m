...
---spat1: Na1 
procedure constructSpat1(Na1:NonceType; Var num: indexType);
...
---spat2: Gateway 
procedure constructSpat2(Gateway:AgentType; Var num: indexType);
  Var i, index : indexType;
  begin
   index:=0;
   i := 1;
...
---spat3: Na1.Gateway 
procedure constructSpat3(Na1:NonceType; Gateway:AgentType; Var num: indexType);
  Var i,index, i1, i2:indexType;
  begin
    index:=0;
    constructSpat1(Na1, i1);
    constructSpat2(Gateway, i2);
    i := 1;
    while(i<= msg_end) do
      if (msgs[i].msgType = concat & msgs[i].length = 2) then
        if (msgs[i].concatPart[1] = i1 & msgs[i].concatPart[2] = i2) then
...
---spat4: Host.Na2 
procedure constructSpat4(Host:AgentType; Na2:NonceType; Var num: indexType);
  Var i,index, i1, i2:indexType;
  begin
    index:=0;
    constructSpat2(Host, i1);
    constructSpat1(Na2, i2);
    i := 1;
    while(i<= msg_end) do
      if (msgs[i].msgType = concat & msgs[i].length = 2) then
        if (msgs[i].concatPart[1] = i1 & msgs[i].concatPart[2] = i2) then
...
---spat5: sk(Host) 
procedure constructSpat5(HostSk :AgentType; Var num: indexType);
  Var i, index : indexType;
  begin
   index:=0;
   i := 1;
...
---spat6: aenc{< Host.Na2 >}sk(Host) 
procedure constructSpat6(Host:AgentType; Na2:NonceType; HostSk :AgentType; Var num: indexType);
  Var i,index,i1,i2:indexType;
  begin
    index:=0;
    constructSpat4(Host, Na2, i1);
    constructSpat5(HostSk, i2);
    i := 1;
    while(i <= msg_end) do
      if (msgs[i].msgType = aenc) then
        if (msgs[i].aencMsg = i1 & msgs[i].aencKey = i2) then
...
---spat7: Host.Na2.aenc{< Host.Na2 >}sk(Host) 
procedure constructSpat7(Host:AgentType; Na2:NonceType; HostSk :AgentType; Var num: indexType);
  Var i,index, i1, i2, i3:indexType;
  begin
    index:=0;
    constructSpat2(Host, i1);
    constructSpat1(Na2, i2);
    constructSpat6(Host, Na2, HostSk, i3);
    i := 1;
    while(i<= msg_end) do
      if (msgs[i].msgType = concat & msgs[i].length = 3) then
        if (msgs[i].concatPart[1] = i1 & msgs[i].concatPart[2] = i2 & msgs[i].concatPart[3] = i3) then
...
ruleset i: msgLen do
  ruleset j: roleServerNums do
    rule "intruderEmitMsgIntoCh[3]"
      ch[3].empty=true & i <= pat7Set.length & pat7Set.content[i] != 0 & Spy_known[pat7Set.content[i]] & matchPat(msgs[pat7Set.content[i]], sPat7Set)
      ==>
      begin
...
ruleset i: msgLen do
  ruleset j: roleGatewayNums do
    rule "intruderEmitMsgIntoCh[6]"
      ch[6].empty=true & i <= pat7Set.length & pat7Set.content[i] != 0 & Spy_known[pat7Set.content[i]] & matchPat(msgs[pat7Set.content[i]], sPat7Set)
      ==>
      begin
        if (!emit[pat7Set.content[i]]) then  --- & msgs[msgs[pat7Set.content[i]].aencKey].k.ag=roleGateway[j].Gateway
          clear ch[6];
...
ruleset i: msgLen do
  ruleset j: roleServerNums do
    rule "intruderEmitMsgIntoCh[7]"
      ch[7].empty=true & i <= pat7Set.length & pat7Set.content[i] != 0 & Spy_known[pat7Set.content[i]] & matchPat(msgs[pat7Set.content[i]], sPat7Set)
      ==>
      begin
        if (!emit[pat7Set.content[i]]) then  --- & msgs[msgs[pat7Set.content[i]].aencKey].k.ag=roleServer[j].Server
          clear ch[7];
...
ruleset i: msgLen do
  ruleset j: roleGatewayNums do
    rule "intruderEmitMsgIntoCh[8]"
      ch[8].empty=true & i <= pat7Set.length & pat7Set.content[i] != 0 & Spy_known[pat7Set.content[i]] & matchPat(msgs[pat7Set.content[i]], sPat7Set)
      ==>
      begin
        if (!emit[pat7Set.content[i]]) then  --- & msgs[msgs[pat7Set.content[i]].aencKey].k.ag=roleGateway[j].Gateway
          clear ch[8];
...
ruleset i: msgLen do
  ruleset j: roleHostNums do
    rule "intruderEmitMsgIntoCh[9]"
      ch[9].empty=true & i <= pat7Set.length & pat7Set.content[i] != 0 & Spy_known[pat7Set.content[i]] & matchPat(msgs[pat7Set.content[i]], sPat7Set)
      ==>
      begin
        if (!emit[pat7Set.content[i]]) then  --- & msgs[msgs[pat7Set.content[i]].aencKey].k.ag=roleHost[j].Host
          clear ch[9];
...
ruleset i0: msgLen do
  ruleset i1: msgLen do 
    rule "enconcat 3"	---pat3
      ...
      matchPat(msgs[construct3By12(pat1Set.content[i0],pat2Set.content[i1])], sPat3Set)&
...
ruleset i0: msgLen do
  ruleset i1: msgLen do 
    rule "enconcat 4"	---pat4
      ...
      matchPat(msgs[construct4By21(pat2Set.content[i0],pat1Set.content[i1])], sPat4Set)&
...
ruleset i0: msgLen do
  ...
  ruleset i2: msgLen do 
    rule "enconcat 7"	---pat7
      ...
      matchPat(msgs[construct7By216(pat2Set.content[i0],pat1Set.content[i1],pat6Set.content[i2])], sPat7Set)&
...
startstate
  ...
  for i : roleHostNums do
    constructSpat3(roleHost[i].Na1, roleHost[i].Gateway, gnum);
  endfor;
  for i : roleGatewayNums do
    constructSpat7(roleGateway[i].Host, roleGateway[i].Na2, roleGateway[i].Host, gnum);
  endfor;
  for i : roleServerNums do
    constructSpat7(roleServer[i].Host, roleServer[i].Na2, roleServer[i].Host, gnum);
  endfor;
  for i : roleGatewayNums do
    constructSpat7(roleGateway[i].Server, roleGateway[i].Na3, roleGateway[i].Server, gnum);
  endfor;
  for i : roleHostNums do
    constructSpat7(roleHost[i].Server, roleHost[i].Na3, roleHost[i].Server, gnum);
  endfor;
  for i : roleGatewayNums do
    constructSpat7(roleGateway[i].Host, roleGateway[i].Na3, roleGateway[i].Host, gnum);
  endfor;
  for i : roleServerNums do
    constructSpat7(roleServer[i].Host, roleServer[i].Na3, roleServer[i].Host, gnum);
  endfor;
  for i : roleGatewayNums do
    constructSpat7(roleGateway[i].Server, roleGateway[i].Na4, roleGateway[i].Server, gnum);
  endfor;
  for i : roleHostNums do
    constructSpat7(roleHost[i].Server, roleHost[i].Na4, roleHost[i].Server, gnum);
  endfor;
